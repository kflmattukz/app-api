...
export class ProfilesService {
  constructor(
    @Inject(DRIZZLE_DATABASE) private readonly drizzle: NodePgDatabase<typeof schema>
  ) {}
  ...
  async createProfile(userId: number, profile: typeof schema.profiles.$inferInsert) {
    const isExist = await this.isProfileExist(userId);
    if (isExist) {
    ...
    try {
      ...
      const user_id: { userId: number }[] = await this.drizzle.insert(schema.profiles).values(userProfile).returning({
  ...
  async updateProfile(userId: number, profile: Partial<typeof schema.profiles.$inferInsert>) {
    const isExist = await this.isProfileExist(userId);
    if (!isExist) {
      throw new NotFoundException();
    }
    try {
      const user_id: {userId: number}[] = await this.drizzle.update(schema.profiles).set(profile).where(eq(schema.profiles.userId, userId)).returning({
        userId: schema.profiles.userId
      })
